...
export default function MainDigitalSection(props: ProductListProps) {
  ...
  return (
    <SectionContainer>
      <SectionContainerTitle theme={props.theme}>디지털</SectionContainerTitle>
      <SectionWrapper>
        {props.productList.map((product, i) => {
          const productUrl = `/product/${product.id}`;
          if (product.category === `electronics` && product.id < 13) {
            return (
              <LinkContainer
                to={productUrl}
                key={product.id}
